...
if (isset($_POST['submit'])) {
    ...
    if (empty($name) || empty($mailFrom) || empty($message)) {
        header("Location: index.html?form=empty&name=" . $name . "&email=" . $mailFrom);
        exit();
    } else if (!filter_var($mailFrom, FILTER_VALIDATE_EMAIL)) {
        exit();
        header("Location: index.html?form=invalid_email&name=" . $name);
    } else if (!preg_match("/^[a-zA-Z]*$/", $name)) {
        header("Location: index.html?form=invalid_name&email=" . $email);
        exit();
    }
